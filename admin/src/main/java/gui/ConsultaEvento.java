...
public class ConsultaEvento extends JInternalFrame {
	...
	private void showEdicionDetails() {
		...
		try {
			DTEdicionDetallada edicionDetallada = edicionController.obtenerDatosDetalladosEdicion(nombreEvento, nombreEdicion);
	...
	private void showEdicionDialog(DTEdicionDetallada edicionDetallada) {
		JDialog dialog = new JDialog((java.awt.Frame) javax.swing.SwingUtilities.getWindowAncestor(this), "Detalle de Edición", true);
		...
		SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
		addLabelValue(headerPanel, gbc, 2, 0, "Fecha Inicio:", dateFormat.format(edicionDetallada.edicion().fechaInicio()));

...
public class SubConsultaRegistroConsultaUsuario extends JDialog {
	 ...
	 * Create the frame.
	 */
	public SubConsultaRegistroConsultaUsuario(IUsuarioController usuarioController, DTRegistro registro) {
		setAlwaysOnTop(true);
		setResizable(true);
		...
		setTitle("Consulta de registro de usuario");
		...
		lblFechaRegistroValue.setHorizontalAlignment(SwingConstants.LEFT);
	...
	private void updateTable(List<DTRegistro> registrations) {
		...
		for (DTRegistro registro : registrations) {
			Object[] rowData = { dateFormat.format(registro.fecha()), String.format("%.2f", registro.costo()),
					registro.nombreEvento(), registro.nombreEdicion(), registro.tipoRegistro().nombre() };
	...
	private void showRegistrationDetails() {
		...
		try {
			DTRegistroDetallado detalle = usuarioController.obtenerRegistroDetallado(registro.nicknameAsistente(),
	...
	private void updateDetailsPanel(DTRegistroDetallado detalle) {
		...
		if (detalle.patrocinio().isPresent()) {
			...
			String patrocinioText = String.format("%s - %s (Nivel: %s)", patrocinio.institucion().nombre(),
					patrocinio.codigo(), patrocinio.nivel().toString());

...
public class AltaTipoRegistro extends JInternalFrame implements IReceiver {
	...
	private void saveAction(ActionEvent e) {
		try {
			if(cbxEdicion.getSelectedItem() == null) throw new ValidationInputException("Debe seleccionar una edición");
			String nombreEdicion = cbxEdicion.getSelectedItem().toString();
			String nombreTipoRegistro = txtNombre.getText();
			if(nombreTipoRegistro.isBlank()) throw new ValidationInputException("Debe ingresar un nombre para el tipo de registro");
			float costo = Float.parseFloat(spnCosto.getValue().toString());
			int cupo = Integer.parseInt(spnCupo.getValue().toString());
			String descripcion = txtDescripcion.getText();
			if(descripcion.isBlank()) throw new ValidationInputException("Debe ingresar una descripción para el tipo de registro");
